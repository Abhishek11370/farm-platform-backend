...
if (isConfigured) {
  ...
  logger.warn('Cloudinary not configured. Image uploads will return placeholder fallback URLs.');
}
 ...
 */
export const uploadImage = async (fileStr: string, folder = 'farm-platform'): Promise<string> => {
  if (!isConfigured) {
    logger.debug('Cloudinary not configured, returning mock placeholder image.');
