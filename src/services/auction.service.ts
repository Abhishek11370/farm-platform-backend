...
export class AuctionService {
  static async listAuctions(filters: any) {
  ...
  static async updateAuction(auctionId: string, userId: string, data: any) {
    const auction = await prisma.auction.findUnique({ where: { id: auctionId }, include: { product: true } });
    if (!auction) throw new Error('Auction not found');
    if (auction.product.ownerId !== userId) throw new Error('Unauthorized');
  ...
  static async placeBid(auctionId: string, bidderId: string, amount: number) {
    ...
    if (!auction) throw new Error('Auction not found');
    if (auction.status !== AuctionStatus.LIVE) throw new Error('Auction is not live');
